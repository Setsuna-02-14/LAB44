...
//! ncopy task.
//!
//! The initial version of this file was seq_std.rs; here we replace the
//! single-stage SEQ with a pipelined skeleton (pipe_std) so the ncopy
//! implementation can exploit parallel hardware and reduce arch cost.
//!
...
/// The `pipe_std`-like pipeline stages used by the ncopy grading architecture.
...
// Specify the program counter by an intermediate signal.
// We choose f_pc so debugger sees the fetch-stage PC.
...
// Map stage aliases: F => f, D => d, E => e, M => m, W => w
